...
if [ -d "$PDF_INPUT" ]; then
  ...
  if [ -f "$PDF_DIR/$TXT_NAME" ]; then
    echo "Processing $PDF_DIR/$TXT_NAME with read_mmwr.py"
    ./read_mmwr.py --input "$PDF_DIR/$TXT_NAME" --paragraph
  else
    echo "No output file for $PDF_INPUT found in $PDF_DIR"
  fi
else
