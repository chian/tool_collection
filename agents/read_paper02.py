...
def classify_sentence(sentence, index, topic_list, agents, topic_ider, llm):
    task1 = Task(
        description=f"Classify the following sentence into one of the given topics:\n\nSentence: {sentence}\n\nTopics: {', '.join([topic for _, topic in topic_list])}",
...
def agent_the_paper(sentences,llm):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=NUM_LLM_THREADS) as executor:
        ...
        futures = [executor.submit(classify_sentence, sentence, index, topic_list, agents, topic_ider, llm)
