...
def main():
    ...
    #For each answer, determine if the question was answered. If not, then do a google search.
    completeness_prompt = textwrap.dedent(
        """
        ...
        Is already answered in the below:

        {parsed_answers}

        Please return only the corresponding all caps identifier for unanswered questions
        in python list format. Do not include anything before or after the python list.
        Do not tell me "This is the list" or anything else. Only return something I can
        set equal to a python variable and have it become a list.
        """
        )

    completeness_prompt = completeness_prompt.format(parsed_answers=parsed_answers)
    print(completeness_prompt)

    client=anthropic.Anthropic()
    completeness_response = client.messages.create(
        model="claude-3-5-sonnet-20240620",
        max_tokens=4096,
        temperature=0,
        messages=[
    ...
    response = completeness_response.content
    
    # Extract the text content
    import ast
    remaining_questions = ast.literal_eval(response[0].text)
    ...
    for entity in remaining_questions:
