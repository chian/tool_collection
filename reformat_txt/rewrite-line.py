...
def process_file_line_by_line(file_path, specific_instruction):
    """Process a single file line by line to apply a specific instruction."""
    ...
    with open(file_path, 'r') as file:
        for line in file:
            line_count += 1
            processed_line = process_chunk(line, specific_instruction)
            line_data = {"line": line.strip(), "output": processed_line}
            print(line_data)
...
def main():
    ...
    if os.path.isdir(input_path):
        ...
        process_file_line_by_line(input_path, specific_instruction)
    else:
